...
local function find_cargo_root()
    ...
    if not base_search_path or base_search_path == '' then
        vim.notify("Could not determine a valid starting directory for Cargo project search.", vim.log.levels.WARN,
            { title = "Cargo" })
        return nil
    end
    ...
    vim.notify(
        "Cargo.toml not found in '" .. base_search_path .. "' or its two direct parent directories.",
        vim.log.levels.INFO,
        { title = "Cargo" }
    )
    return nil
...
vim.keymap.set('n', '<leader>lrr', '<cmd>lua _G.RUST_CARGO_RUN()<CR>',
    { noremap = true, silent = true, desc = 'Cargo [R]un' })
vim.keymap.set('n', '<leader>lrc', '<cmd>lua _G.RUST_CARGO_CHECK()<CR>',
    { noremap = true, silent = true, desc = 'Cargo [C]heck' })
